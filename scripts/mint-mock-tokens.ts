...
class MockTokenMinter {
  ...
  constructor() {
    ...
    if (!this.srcUSDCAddress || !this.dstWETHAddress) {
      throw new Error(
        "Mock token addresses not found in config. Please run 'yarn setup:mock-tokens' first."
      );
    }
  ...
  async mintUSDCToUser(): Promise<void> {
    ...
    try {
      ...
      // Call mint function
      const tx = await mockUSDCContract.mint(
  ...
  async mintWETHToResolver(): Promise<void> {
    ...
    try {
      ...
      // Call mint function
      const tx = await mockWETHContract.mint(
        this.resolverWallet.address,
        mintAmount
      );
      console.log(`  ⏳ Transaction hash: ${tx.hash}`);
      ...
      // Check balance after minting
      const balance = await mockWETHContract.balanceOf(
        this.resolverWallet.address
      );
      console.log(
      ...
      console.log(
        `  🔗 View on Arbiscan: https://sepolia.arbiscan.io/tx/${tx.hash}`
      );
    } catch (error) {
  ...
  async checkBalances(): Promise<void> {
    ...
    try {
      ...
      const usdcBalance = await mockUSDCContract.balanceOf(
        this.userWallet.address
      );
      console.log(
        `  💰 User USDC balance: ${ethers.formatUnits(usdcBalance, 6)} USDC`
      );
      ...
      const wethBalance = await mockWETHContract.balanceOf(
        this.resolverWallet.address
      );
      console.log(
        `  💰 Resolver WETH balance: ${ethers.formatEther(wethBalance)} WETH`
      );
    } catch (error) {
  ...
  async runMinting(): Promise<void> {
    try {
      ...
      console.log("\n📝 Summary:");
      console.log(
        `  👤 User (${this.userWallet.address}): 1000 USDC on Sepolia`
      );
      console.log(
        `  🔧 Resolver (${this.resolverWallet.address}): 100 WETH on Arbitrum`
      );
      console.log("\n📋 Next Steps:");
