...
export class Wallet {
    ...
    constructor(privateKeyOrSigner: string | Signer, provider: JsonRpcProvider) {
        ...
        this.signer = typeof privateKeyOrSigner === 'string' ? new PKWallet(privateKeyOrSigner, this.provider) : privateKeyOrSigner;
    }

    public static async contractWithSigner(contractAddress: string, privateKeyOrSigner: string | Signer, provider: JsonRpcProvider): Promise<Wallet> {
        const wallet = new Wallet(privateKeyOrSigner, provider);
        wallet.contractAddress = contractAddress; // Set the contract address
        return wallet;
    }

    async tokenBalance(token: string): Promise<bigint> {
        const tokenContract = new Contract(token.toString(), ERC20.abi, this.provider);
        // add for dealing with smart contract
        if (this.contractAddress !== undefined && this.contractAddress !== null && this.contractAddress !== '') {
            return tokenContract.balanceOf(this.contractAddress.toString());
        }
