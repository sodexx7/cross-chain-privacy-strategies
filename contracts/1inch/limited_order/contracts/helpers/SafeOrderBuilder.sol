...
contract SafeOrderBuilder is GnosisSafeStorage {
    ...
    bytes32 private constant _SAFE_MSG_TYPEHASH =
    ...
    constructor(
        ...
        IOrderRegistrator orderRegistrator
    ) {
    ...
    function buildAndSignOrder(
    ...
    ) external {
        {
            ...
            ) = makerAssetOracleParams.oracle.latestRoundData();
            // solhint-disable-next-line not-rely-on-time
            if (updatedAt + makerAssetOracleParams.ttl < block.timestamp)
                revert StaleOraclePrice();
            order.takingAmount = order.takingAmount.mulDiv(
                uint256(latestAnswer),
                makerAssetOracleParams.originalAnswer
            );
        }

        {
            ...
            ) = takerAssetOracleParams.oracle.latestRoundData();
            // solhint-disable-next-line not-rely-on-time
            if (updatedAt + takerAssetOracleParams.ttl < block.timestamp)
                revert StaleOraclePrice();
            order.takingAmount = order.takingAmount.mulDiv(
                takerAssetOracleParams.originalAnswer,
                uint256(latestAnswer)
            );
        }

        bytes32 msgHash = _getMessageHash(
            abi.encode(_LIMIT_ORDER_PROTOCOL.hashOrder(order))
        );
    ...
    function _getMessageHash(
        ...
        bytes32 safeMessageHash = keccak256(
            abi.encode(_SAFE_MSG_TYPEHASH, keccak256(message))
        );
        return
            keccak256(
                abi.encodePacked(
