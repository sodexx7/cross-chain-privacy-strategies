...
contract PrivacyResolver is Resolver {
    ...
    constructor(IEscrowFactory factory, IOrderMixin lop, address initialOwner) Resolver(factory, lop, initialOwner) {}
     ...
     */
    function commitOrder(bytes32 commitHash, uint256 revealDelay) external onlyOwner {
     ...
     */
    function revealAndScheduleOrder(bytes calldata orderData, uint256 nonce, uint256 executionDelay) external onlyOwner {
        // Verify commitment
    ...
    function deployPrivateSrc(
        ...
        modifiedImmutables.timelocks = TimelocksLib.setDeployedAt(immutables.timelocks, block.timestamp + randomDelay);
     ...
     */
    function getPrivacyStats() external view returns (uint256 totalCommitments, uint256 revealedCommitments, uint256 totalFakeVolume) {
     ...
     */
    function generateCommitHash(bytes calldata orderData, uint256 nonce, address committer) external pure returns (bytes32) {
