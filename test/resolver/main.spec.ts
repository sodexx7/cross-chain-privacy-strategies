...
import { computeAddress, ContractFactory, JsonRpcProvider, MaxUint256, parseEther, parseUnits, randomBytes, Wallet as SignerWallet } from 'ethers';
...
describe('Resolving example', () => {
    ...
    before(async function () {
        ...
        await src.provider.send('anvil_setBalance', [await srcChainResolver.getAddress(), '0x56BC75E2D630E000']); // 100 ETH
        await dst.provider.send('anvil_setBalance', [await dstChainResolver.getAddress(), '0x56BC75E2D630E000']); // 100 ETH
    ...
    describe('Fill', () => {
        it('should swap Ethereum USDC -> Bsc USDC. Single fill only', async function () {
            ...
            const initialBalances = await getBalances(config.chain.source.tokens.USDC.address, config.chain.destination.tokens.USDC.address);
            ...
            const { txHash: dstDepositHash, blockTimestamp: dstDeployedAt } = await dstChainResolver.send(resolverContract.deployDst(dstImmutables));
            ...
            console.log(`[${srcChainId}]`, `Withdrew funds for resolver from ${srcEscrowAddress} to ${src.resolver} in tx ${resolverWithdrawHash}`);

            const resultBalances = await getBalances(config.chain.source.tokens.USDC.address, config.chain.destination.tokens.USDC.address);

            // user transferred funds to resolver on source chain
            expect(initialBalances.src.user - resultBalances.src.user).to.equal(order.makingAmount);
            expect(resultBalances.src.resolver - initialBalances.src.resolver).to.equal(order.makingAmount);
            // resolver transferred funds to user on destination chain
            expect(resultBalances.dst.user - initialBalances.dst.user).to.equal(order.takingAmount);
            expect(initialBalances.dst.resolver - resultBalances.dst.resolver).to.equal(order.takingAmount);
        });
        ...
        it('should swap Ethereum USDC -> Bsc USDC. Multiple fills. Fill 50%', async function () {
            ...
            // user transferred funds to resolver on the source chain
            expect(initialBalances.src.user - resultBalances.src.user).to.equal(fillAmount);
            expect(resultBalances.src.resolver - initialBalances.src.resolver).to.equal(fillAmount);
            // resolver transferred funds to user on the destination chain
            const dstAmount = (order.takingAmount * fillAmount) / order.makingAmount;
            expect(resultBalances.dst.user - initialBalances.dst.user).to.equal(dstAmount);
            expect(initialBalances.dst.resolver - resultBalances.dst.resolver).to.equal(dstAmount);
        });
    });
    ...
    describe('Cancel', () => {
        it('should cancel swap Ethereum USDC -> Bsc USDC', async function () {
            ...
            await dstChainResolver.send(resolverContract.cancel('dst', dstEscrowAddress, dstImmutables.withDeployedAt(dstDeployedAt)));

            console.log(`[${srcChainId}]`, `Cancelling src escrow ${srcEscrowAddress}`);
            const { txHash: cancelSrcEscrow } = await srcChainResolver.send(resolverContract.cancel('src', srcEscrowAddress, srcEscrowEvent[0]));
 ...
 */
async function deploy(json: { abi: any; bytecode: any }, params: unknown[], provider: JsonRpcProvider, deployer: SignerWallet): Promise<string> {
